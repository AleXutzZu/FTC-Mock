...
public class OpModeStarterImpl implements OpModeStarter {
    private final HardwareMap hardwareMap;
    private final Telemetry telemetry;
    private List<String> lastDisplay;

    OpModeStarterImpl(HardwareMap hardwareMap) {
        this.hardwareMap = hardwareMap;
        Class<TelemetryImpl> telemetryClass = TelemetryImpl.class;
        try {
            telemetryClass.getDeclaredConstructor().setAccessible(true);
        } catch (NoSuchMethodException e) {
            throw new RuntimeException(e);
        }
        try {
            telemetry = telemetryClass.newInstance();
